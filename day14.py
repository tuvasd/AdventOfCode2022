...
def makeCave(input, minAndMax):
    ...
    print(minAndMax[3])
    grid = []
    for _ in range(0,minAndMax[3]+1):
        tmp = []
        for _ in range(minX,minAndMax[2]+1):
            tmp.append(".")
        grid.append(tmp)
    ...
    for line in input:
        for i in range(0,len(line)-1):
            ...
            x1_offset = first[0] - minX
            x2_offset = second[0] - minX
            if first[0] != second[0]:
...
def movingOne(currentX, currentY,grid):
    ...
    for item in moving:
        if nextLine[currentX + item[0]] == ".":
            ...
            if currentX == -1 or currentX == len(grid[currentY]):
                return False
            currentY = currentY + 1
